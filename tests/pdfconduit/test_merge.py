...
def merge_params():
    ...
    return [(test_data_path(main_pdf), list(map(lambda pdf: test_data_path(pdf), to_merge)))
...
class TestMerge(PdfconduitTestCase):
    ...
    @parameterized.expand(merge_params, name_func=merge_name_func)
    def test_can_merge_pdfs_using_context(self, main_pdf: str, pdfs_to_merge: Iterable[str]):
        self.pdf_path = main_pdf
    ...
    def assertCorrectNumPages(self, main_pdf: str, pdfs_to_merge: Iterable[str], expected_pages: int) -> None:
        # Assert sum of pages in original pdf files equals sum of pages in merged pdf
        self.assertEqual(sum([Info(pdf).pages for pdf in pdfs_to_merge]) + Info(main_pdf).pages, expected_pages)
