...
def main():
    ...
    ap.add_argument('--times', action="store_true", help='Sort test results by execution time')
    ap.add_argument('--ascending', action="store_true", help='Order test results in ascending order')
    args = ap.parse_args()
    ...
    if args.times:
        data = dict(sorted(data.items(), key=lambda item: item[1], reverse=args.ascending if args.ascending else False))
